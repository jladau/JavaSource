package edu.ucsf.WindowedSlopes;
...
public class WindowedSlopesLauncher{
	...
	public static void main(String rgsArgs[]){
		...
		lstOut.add("CATEGORY,X_MIN,X_MAX,X_MEAN,SLOPE"); 
		...
		for(int i=1;i<dat1.iRows;i++){
			...
			if(i==dat1.iRows-1 || (sCat!=null && !dat1.getString(i,sCat).equals(dat1.getString(i+1,sCat)))){
				for(int l=iMin;l<=i;l++){
					...
					lstOut.add(dat1.getString(i,sCat) + "," + ExtendedMath.minimum(lst1) + "," +  ExtendedMath.maximum(lst1) + "," + ExtendedMath.mean(lst1) + "," + ExtendedMath.slope(lst1,lst2));
				}
				tbl1 = HashBasedTable.create(dat1.iRows,3);
				iMin = Integer.MAX_VALUE;
			}	
