...
public class NetcdfWriterCopy {
	...
	public NetcdfWriterCopy(GeospatialRaster[] rgr1, String sOutputPath) throws Exception{
	...
	public void writeRaster(GeospatialRaster ras1, LocalDate tim1, double dVert) throws Exception{
		...
		if(ras1.hasVert() && ras1.hasTime()){
			...
			ary4 = new ArrayDouble.D4(1, 1, ras1.axsLat.size(), ras1.axsLon.size());
			itr2 = ary4.getIndexIterator();
			while(itr2.hasNext()){
				itr2.next();
				itr2.setDoubleCurrent(-9999.);
			}
			rgiOrigin = new int[]{iTimeIndex,iVertIndex,0,0};
			...
			//writing data
			iTimeIndex=mapTimeIndex.get(axeTime.ID);
			ary3 = new ArrayDouble.D3(1, ras1.axsLat.size(), ras1.axsLon.size());
			itr2 = ary3.getIndexIterator();
			while(itr2.hasNext()){
				itr2.next();
				itr2.setDoubleCurrent(-9999.);
			}
			rgiOrigin = new int[]{iTimeIndex,0,0};
			itr1 = ras1.new LatLonIterator(tim1, dVert);
			while(itr1.hasNext()){
				cel1 = itr1.next();
			...
			rgiOrigin = new int[]{iVertIndex,0,0};
			itr1 = ras1.new LatLonIterator(tim1, dVert);
			while(itr1.hasNext()){
				cel1 = itr1.next();
				ary3.set(0, cel1.axeLat.iID, cel1.axeLon.iID, ras1.get(cel1));	
			...
			//writing data
			ary2 = new ArrayDouble.D2(ras1.axsLat.size(), ras1.axsLon.size());
			itr2 = ary2.getIndexIterator();
			while(itr2.hasNext()){
				itr2.next();
				itr2.setDoubleCurrent(-9999.);
			}
			rgiOrigin = new int[]{0,0};
			itr1 = ras1.new LatLonIterator(tim1, dVert);
			while(itr1.hasNext()){
				cel1 = itr1.next();
