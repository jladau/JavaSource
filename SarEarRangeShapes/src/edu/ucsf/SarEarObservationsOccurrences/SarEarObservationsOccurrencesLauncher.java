package edu.ucsf.SarEarObservationsOccurrences;
...
public class SarEarObservationsOccurrencesLauncher {
	...
	public static void main(String rgsArgs[]) throws Exception{
		...
		for(int k=1;k<dat1.iRows;k++){	
			mapOccurrences.put(dat1.getString(k, "STUDY_REGION_ID"), dat1.getString(k, "SPECIES_ID"));
		}
		...
		//outputting results
		DataIO.writeToFile(
				loadResults(shp1, rgcRegions, mapOccurrences),
	...
	private static ArrayList<String> loadResults(
		...
		for(int k=0;k<rgcRegions.length;k++){
			...
			if(rgcRegions[k].intersectsMerged()){
				...
				HashSet<String> set1 = new HashSet<String>(mapOccurrences.get(rgcRegions[k].getID()));
				for(String s:set1){
