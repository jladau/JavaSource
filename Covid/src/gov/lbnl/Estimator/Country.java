...
public class Country extends Region{
	...
	public Country(int iStartTime, int iEndTime, ArrayList<Integer> lstTimes){
		super(null, iStartTime, iEndTime, lstTimes);
	}
	
	public Country(Country ctr1){
		super(ctr1);
	...
	public ArrayList<String> print(
		...
		if(bTimeIntegrated) {
			return printTimeIntegrated(rgsCountyVariables, rgsStateVariables, rgsCountryVariables, sType, bHeader);
		}
	...
	private ArrayList<String> printTimeIntegrated(
			String rgsCountyVariables[], 
			String rgsStateVariables[], 
			String rgsCountryVariables[],
			String sType,
		...
		//lstOut = output
		//set1 = set of times to consider
		//dSum = current time integrated sum
		//d1 = current value
		...
		for(State sta1:states()){
			for(County cty1:sta1.counties()){
				for(String sVar:rgsCountyVariables){
					if(sVar.equals("odds_ratio") || sVar.equals("infection_fatality_rate")) {
						for(Integer iTime:set1){
							d1 = cty1.get(sVar,iTime,sType);
							if(Double.isNaN(d1)) {
								continue;
							}
							if(sVar.equals("infection_fatality_rate")) {	
								lstOut.add(cty1.sFips + "," + sta1.sFips + "," + sVar + "," + sType + "," + "NA" + "," + d1);
								break;
							}else {
								lstOut.add(cty1.sFips + "," + sta1.sFips + "," + sVar + "," + sType + "," + iTime + "," + d1);
							}
						...
						for(Integer iTime:set1){	
							if(cty1.contains(sVar,iTime,sType)){
								dSum+=cty1.get(sVar,iTime,sType);		
							}
						}
						lstOut.add(cty1.sFips + "," + sta1.sFips + "," + sVar + "," + sType + "," + "NA" + "," + dSum);
					}
				}
			}
			for(String sVar:rgsStateVariables){
				if(sVar.equals("odds_ratio") || sVar.equals("infection_fatality_rate")) {
					for(Integer iTime:set1){
						...
						if(sVar.equals("infection_fatality_rate")) {	
							lstOut.add("NA" + "," + sta1.sFips + "," + sVar + "," + sType + "," + "NA" + "," + d1);
							break;
						}else {
							lstOut.add("NA" + "," + sta1.sFips + "," + sVar + "," + sType + "," + iTime + "," + d1);
						}
					...
					for(Integer iTime:set1){	
						if(sta1.contains(sVar,iTime,sType)){
							dSum += sta1.get(sVar,iTime,sType);
						}
					}
					lstOut.add("NA" + "," + sta1.sFips + "," + sVar + "," + sType + "," + "NA" + "," + dSum);
				}
			}
		}
		for(String sVar:rgsCountryVariables){
			if(sVar.equals("odds_ratio") || sVar.equals("infection_fatality_rate")) {
				for(Integer iTime:set1){
					...
					if(sVar.equals("infection_fatality_rate")) {							
						lstOut.add("NA" + "," + "NA" + "," + sVar + "," + sType + "," + "NA" + "," + d1);
						break;
					}else {
						lstOut.add("NA" + "," + "NA" + "," + sVar + "," + sType + "," + iTime + "," + d1);
					}
				...
				for(Integer iTime:set1){	
					if(this.contains(sVar,iTime,sType)){
						dSum += this.get(sVar,iTime,sType);
					}
				}
				lstOut.add("NA" + "," + "NA" + "," + sVar + "," + sType + "," + "NA" + "," + dSum);
			}
		}
		return lstOut;
	}
